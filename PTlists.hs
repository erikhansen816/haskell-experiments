...
allIntegersInclusive a b = if b>a then [a..b] else [a,a-1..b]
...
{-
  ...
  of the problem.
-}
stringList n = [x:[] | x <- (show n)]
stringToNumber n = [(read :: String -> Int) x | x <- stringList n]
isRigid n = sum (stringToNumber n) == product (stringToNumber n)
...
fizz n f = [x | x <- [1..n-1], x `mod` f == 0]

buzz n b = [x | x <- [1..n-1], x `mod` b == 0]
